/**
 * Stitch MCP Client — ESM version
 * Connects to https://stitch.googleapis.com/mcp using @modelcontextprotocol/sdk
 */
...
const SCREEN_DIR = join(__dirname, '..', 'design', 'screens');
...
async function main() {
  ...
  try {
    ...
    console.log('✅ Project:', JSON.stringify(projectResult).substring(0, 400));
  } catch (e) {
  ...
  const projectId = projectResult?.projectId || projectResult?.id || 'suri-learning-app';
        ...
        if (screenId) {
          try {
            const screenResult = await client.callTool(
              { name: 'get_screen', arguments: { screenId, projectId } },
              undefined,
              { timeout: 60000 },
            );
            const screenObj = typeof screenResult === 'string' ? JSON.parse(screenResult) : screenResult;
            html = screenObj?.html || screenObj?.htmlUrl || null;
            imageUrl = screenObj?.imageUrl || screenObj?.screenshotUrl || null;
          } catch (_) {
            // HTML may be in generation result
          }
        }

        if (!html) html = resultObj.html || resultObj.htmlUrl || null;
        if (!imageUrl) imageUrl = resultObj.imageUrl || resultObj.screenshotUrl || null;
      }

      const filename = join(SCREEN_DIR, `${screen.name}-stitch.html`);

      if (html && typeof html === 'string' && html.length > 100) {
        writeFileSync(filename, html);
        console.log(`     📁 Saved: ${screen.name}-stitch.html (${html.length} chars)`);
      } else {
        // Save raw result
        writeFileSync(filename, `<!-- Stitch Result: ${screen.description} -->\n<!-- Screen ID: ${screenId || 'N/A'} -->\n<!-- Generated: ${new Date().toISOString()} -->\n<!-- Raw: ${JSON.stringify(result, null, 2)} -->`);
        console.log(`     📁 Saved raw result (${JSON.stringify(result).length} bytes)`);
      }

      if (imageUrl) console.log(`     🖼️  ${imageUrl}`);
      if (screenId) console.log(`     🆔 ${screenId}`);
